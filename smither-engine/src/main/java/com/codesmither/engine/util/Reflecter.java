...
//        new NoSuchMethodException(method).printStackTrace();
    ...
    public static boolean copyFiled(Object from, Object to) {
        if (from == null || to == null) {
            return false;
        }
        Class<?> toClass = to.getClass();
        Field[] fields = getField(from.getClass());
        for (Field field : fields) {
            try {
                Field toClassField = getField(toClass,field.getName());
                if (toClassField != null) {
                    field.setAccessible(true);
                    toClassField.setAccessible(true);
                    toClassField.set(to,field.get(from));
                }
